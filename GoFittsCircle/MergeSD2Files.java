...
/**
 * <table width="100%" summary="MergeSD2Files title">
 ...
 * 
 * <ul>
 * <li>Utility software to merge all the sd2 files in a directory into a single file for follow-up analyses.
 * </ul>
 * 
 * <h2>Getting Started</h2>
 * 
 * MergeSD2Files is launched from GoFitts by selecting "Merge sd2 files" and clicking "Go" (below, left). MergeSD2Files
 * begins with a launch panel (below, centre). Clicking "Open..." opens a file chooser showing all the sd2 files in the
 * current directory. Navigate to a desired directory, then select any sd2 file and click "Open" (below, right).
 * 
 * <center><p> <a href="MergeSD2Files-0.jpg"><img src="MergeSD2Files-0.jpg" width="200" alt="MergeSD2Files-0"></a>
 * <a href="MergeSD2Files-1.jpg"><img src="MergeSD2Files-1.jpg" width="400" alt="MergeSD2Files-1"></a>
 * <a href="MergeSD2Files-2.jpg"><img src="MergeSD2Files-2.jpg" width="300" alt="MergeSD2Files-2"></a> </center>
 * <p>
 * 
 * Upon selecting an sd2 file and clicking "Open", MergeSD2Files reads the data from all the sd2 files in the directory
 ...
 * for the sd2 data from an experiment that used an early version of GoFitts called FittsTaskTwo.
 * <a href="http://www.yorku.ca/mack/icchp2016b.html">Click here</a> to view the publication where these data are
 * analysed.
 * 
 * <center><p> <a href="MergeSD2Files-3.jpg"><img src="MergeSD2Files-3.jpg" width="800" alt="MergeSD2Files-3"></a></center>
 * <p>
 * 
 ...
 * file. In the example, 72 files were read with 4 data lines per file.
 * <p>
 * 
 ...
 * SD2 files. This option is useful if the data are organized in sub-folders, for example, with each sub-folder holding
 * the data files for a separate participant. However, you must select an ".sd2" file in the parent directory to begin
 * the merge process.  If necessary, create such a file and call it "FittsTask-dummy.sd2".  The dummy file should contain one
 * line only -- the header line from any .sd2 file.
 * <p>
 * 
 * @author Scott MacKenzie, 2017-2024
 */

public class MergeSD2Files extends JFrame implements ActionListener, ItemListener
{
	...
	public MergeSD2Files(File currentDirectoryArg)
	{
		...
		api.setToolTipText(GoFitts.API_LOCATION + "MergeSD2Files.html");

		JLabel directoryLabel = new JLabel("sd2 File");
		JTextField directory = new JTextField(20);

		fcOpen = new JFileChooser(currentDirectory);
		fcOpen.setFileFilter(new FileNameExtensionFilter("sd2 file", "sd2"));
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (source == api)
		{
			boolean okReturned = GoFitts.openBrowser("index.html?MergeSD2Files.html");
			if (!okReturned)
			{
		...
		else if (source == open)
		{
			if (fcOpen.showOpenDialog(this) == JFileChooser.APPROVE_OPTION)
			{
				...
				loadDataFromSd2Files(appName, fcOpen.getCurrentDirectory());
	...
	void loadDataFromSd2Files(String appName, File directory)
	{
		// get an array of all sd2 files for the specified app in the specified directory
		File[] f = getSD2Files(appName, directory.getAbsolutePath());
		...
		String[] tmp = arrayList.get(0).trim().split(",");
		...
		for (int i = 0; i < rows; ++i) // iterator down rows
		{
			...
			// iterate across columns (add entry at beginning for row number)
			for (int j = 0; j < tmp.length + 1; ++j) 
			{
				int rowNumber = i + 1;
				data[i][j] = j == 0 ? "" + rowNumber : tmp[j - 1].trim();
	...
	public File[] getSD2Files(String appName, String directoryName)
	{
		sd2Files = new ArrayList<File>();
		traverse(appName, new File(directoryName), includeSubFolders);
		File[] f = new File[sd2Files.size()];
		return sd2Files.toArray(f);
	}
		...
		if (file.isDirectory())
		{
			...
			if (entryArray != null)
			{
				...
				for (String entry : entryArray)
					if (entry.startsWith(appName) && entry.endsWith(".sd2"))
						sd2Files.add(new File(pathName + File.separator + entry));
